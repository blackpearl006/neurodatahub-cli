...
class TestUserInteraction:
    ...
    def test_get_user_input_keyboard_interrupt(self, mock_input):
        ...
        with patch("neurodatahub.utils.console"), pytest.raises(SystemExit):
...
class TestUtilityFunctions:
    ...
    def test_check_available_space_insufficient(self, temp_dir):
        ...
        with patch("os.statvfs") as mock_statvfs, patch("neurodatahub.utils.console"):
            mock_statvfs.return_value = MagicMock()
            mock_statvfs.return_value.f_frsize = 4096
...
class TestEdgeCases:
    ...
    def test_get_confirmation_various_inputs(self):
        ...
        for input_val, expected in test_cases:
            with patch("builtins.input", return_value=input_val), patch(
                "neurodatahub.utils.console"
            ):
                result = get_confirmation("Test?", default=False)
                assert (
                    result == expected
                ), f"Input '{input_val}' should return {expected}"
