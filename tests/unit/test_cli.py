...
class TestMainCommand:
    ...
    def test_list_both_auth_flags_error(self, cli_runner):
        ...
        with patch("neurodatahub.cli.display_error") as mock_error:
            result = cli_runner.invoke(
                main, ["--list", "--auth-only", "--no-auth-only"]
            )
            mock_error.assert_called_once_with(
                "Cannot use both --auth-only and --no-auth-only"
            )

    def test_pull_dataset_success(self, cli_runner, sample_dataset):
        """Test successful dataset pull."""
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.download_manager"
        ) as mock_dl, patch("neurodatahub.cli.display_dataset_info"), patch(
            "neurodatahub.cli.get_confirmation", return_value=True
        ), patch(
            "neurodatahub.cli.display_success"
        ):
    ...
    def test_pull_dataset_not_found(self, cli_runner):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.display_error"
        ) as mock_error, patch("neurodatahub.cli.display_info") as mock_info:

            mock_dm.get_dataset.return_value = None

            result = cli_runner.invoke(
                main, ["--pull", "NONEXISTENT", "--path", "/tmp/test"]
            )
            mock_error.assert_called_once_with("Dataset 'NONEXISTENT' not found")
            mock_info.assert_called_once_with(
                "Use 'neurodatahub --list' to see available datasets"
            )
    ...
    def test_pull_user_cancellation(self, cli_runner, sample_dataset):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.display_dataset_info"
        ), patch("neurodatahub.cli.get_confirmation", return_value=False), patch(
            "neurodatahub.cli.display_info"
        ) as mock_info:
    ...
    def test_pull_ida_dataset(self, cli_runner, sample_ida_dataset):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.run_ida_workflow"
        ) as mock_ida, patch("neurodatahub.cli.display_dataset_info"), patch(
            "neurodatahub.cli.get_confirmation", return_value=True
        ), patch(
            "neurodatahub.cli.display_success"
        ):
    ...
    def test_pull_auth_required_dataset(self, cli_runner, sample_auth_dataset):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.auth_manager"
        ) as mock_auth, patch("neurodatahub.cli.download_manager") as mock_dl, patch(
            "neurodatahub.cli.display_dataset_info"
        ), patch(
            "neurodatahub.cli.get_confirmation", return_value=True
        ), patch(
            ...
            result = cli_runner.invoke(
                main, ["--pull", "AUTH_DATASET", "--path", "/tmp/test"]
            )
            assert result.exit_code == 0
    ...
    def test_pull_auth_failure(self, cli_runner, sample_auth_dataset):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.auth_manager"
        ) as mock_auth, patch("neurodatahub.cli.display_dataset_info"), patch(
            "neurodatahub.cli.get_confirmation", return_value=True
        ), patch(
            "neurodatahub.cli.display_error"
        ) as mock_error:
    ...
    def test_dry_run_download(self, cli_runner, sample_dataset):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.download_manager"
        ) as mock_dl, patch("neurodatahub.cli.display_dataset_info"):

            mock_dm.get_dataset.return_value = sample_dataset
            mock_dl.download_dataset.return_value = True

            result = cli_runner.invoke(
                main, ["--pull", "HBN", "--path", "/tmp/test", "--dry-run"]
            )
            assert result.exit_code == 0
            # Dry run should not call get_confirmation
            mock_dl.download_dataset.assert_called_once_with(
                sample_dataset, "/tmp/test", True
            )
...
class TestSubcommands:
    ...
    def test_check_command(self, cli_runner):
        ...
        with patch("neurodatahub.cli.display_info") as mock_info, patch(
            "neurodatahub.cli.display_dependency_status"
        ) as mock_status:
    ...
    def test_info_command_success(self, cli_runner, sample_dataset):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.display_dataset_info"
        ) as mock_info, patch("neurodatahub.cli.console"):
    ...
    def test_search_command_no_results(self, cli_runner):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            ...
            assert result.exit_code == 0
            mock_info.assert_called_once_with(
                "No datasets found matching 'nonexistent'"
            )
    ...
    def test_list_command_with_filters(self, cli_runner):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm:
            ...
            result = cli_runner.invoke(
                main, ["list", "--category", "indi", "--auth-required"]
            )
            assert result.exit_code == 0
    ...
    def test_list_command_conflicting_flags(self, cli_runner):
        ...
        with patch("neurodatahub.cli.display_error") as mock_error:
            ...
            mock_error.assert_called_once_with(
                "Cannot use both --auth-required and --no-auth"
            )
    ...
    def test_version_command(self, cli_runner):
        ...
        with patch("neurodatahub.cli.console") as mock_console:
            ...
            )  # Should print version, homepage, and repository
...
class TestErrorHandling:
    ...
    def test_keyboard_interrupt(self, cli_runner, sample_dataset):
        ...
        with patch("neurodatahub.cli.dataset_manager") as mock_dm, patch(
            "neurodatahub.cli.get_confirmation", side_effect=KeyboardInterrupt
        ):
