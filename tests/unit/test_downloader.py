...
class TestBaseDownloader:
    ...
    def test_prepare_success(self, sample_dataset, temp_dir):
        ...
        with patch("neurodatahub.downloader.validate_path", return_value=True), patch(
    ...
    def test_prepare_insufficient_space(self, sample_dataset, temp_dir):
        ...
        with patch("neurodatahub.downloader.validate_path", return_value=True), patch(
            "neurodatahub.downloader.check_available_space", return_value=False
        ):
            downloader = BaseDownloader(sample_dataset, str(temp_dir))
            assert downloader.prepare() is False
...
class TestAwsS3Downloader:
    ...
    def test_prepare_missing_aws_cli(self, sample_dataset, temp_dir):
        ...
        with patch("neurodatahub.downloader.validate_path", return_value=True), patch(
            "neurodatahub.downloader.check_available_space", return_value=True
        ), patch("neurodatahub.downloader.check_dependency", return_value=False), patch(
    ...
    def test_prepare_missing_credentials(self, sample_auth_dataset, temp_dir):
        ...
        with patch("neurodatahub.downloader.validate_path", return_value=True), patch(
            "neurodatahub.downloader.check_available_space", return_value=True
        ), patch("neurodatahub.downloader.check_dependency", return_value=True), patch(
            "neurodatahub.downloader.run_command", return_value=(1, "", "error")
        ), patch(
            "neurodatahub.downloader.display_error"
        ):
            downloader = AwsS3Downloader(sample_auth_dataset, str(temp_dir))
    ...
    def test_download_success(self, sample_dataset, temp_dir):
        ...
        ), patch("neurodatahub.downloader.display_info"), patch(
...
class TestAria2cDownloader:
    ...
    def test_download_single_file(self, temp_dir):
        ...
        with patch(
            "neurodatahub.downloader.run_command", return_value=(0, "", "")
        ), patch("neurodatahub.downloader.display_info"), patch(
            "neurodatahub.downloader.display_success"
        ), patch(
            "time.time", side_effect=[0, 10]
    ...
    def test_download_multiple_files_oasis1(self, temp_dir):
        ...
        dataset = {"name": "OASIS1", "base_command": "multiple_aria2c_downloads"}
        with patch(
            "neurodatahub.downloader.run_command", return_value=(0, "", "")
    ...
    def test_download_unknown_multiple(self, temp_dir):
        ...
        dataset = {"name": "UNKNOWN", "base_command": "multiple_aria2c_downloads"}
        with patch("neurodatahub.downloader.display_error"):
            downloader = Aria2cDownloader(dataset, str(temp_dir))
            assert downloader.download() is False
...
class TestDataladDownloader:
    ...
    def test_prepare_missing_dependencies(self, temp_dir):
        ...
        with patch("neurodatahub.downloader.validate_path", return_value=True), patch(
            ...
            "neurodatahub.downloader.check_dependency", side_effect=mock_check_dep
        ), patch(
            "neurodatahub.downloader.display_error"
        ):
            downloader = DataladDownloader(dataset, str(temp_dir))
    ...
    def test_download_success(self, temp_dir):
        ...
        with patch(
            ...
            "os.getcwd", return_value=original_cwd
        ), patch(
            "os.chdir"
        ):
            downloader = DataladDownloader(dataset, str(temp_dir))
    ...
    def test_download_no_repository(self, temp_dir):
        ...
        dataset = {"name": "Test Dataset", "repository": ""}
        with patch("neurodatahub.downloader.display_error"):
            downloader = DataladDownloader(dataset, str(temp_dir))
            assert downloader.download() is False
...
class TestRequestsDownloader:
    ...
    def test_download_success(self, sample_dataset, temp_dir):
        ...
        ), patch("neurodatahub.downloader.display_success"), patch(
            "tqdm.tqdm"
        ) as mock_tqdm:
...
class TestDownloadManager:
    ...
    def test_download_dataset_exception(self, sample_dataset, temp_dir):
        ...
        with patch.object(
            manager, "get_downloader", return_value=mock_downloader
    ...
    def test_download_dataset_no_downloader(self, sample_dataset, temp_dir):
        ...
        with patch.object(manager, "get_downloader", return_value=None), patch.object(
            manager, "_try_fallback_download", return_value=False
        ):
            result = manager.download_dataset(sample_dataset, str(temp_dir))
            assert result is False
    ...
    def test_try_fallback_download_with_url(self, temp_dir):
        ...
        with patch("neurodatahub.downloader.display_info"), patch(
            "neurodatahub.downloader.RequestsDownloader"
    ...
    def test_try_fallback_download_no_url(self, temp_dir):
        ...
        dataset = {"name": "Test", "base_command": "some command without url"}
        manager = DownloadManager()
...
class TestEdgeCases:
    ...
    def test_aws_downloader_command_with_spaces(self, temp_dir):
        ...
        dataset = {"name": "Test", "base_command": "aws s3 sync s3://bucket/ ."}
    ...
    def test_aria2c_multiple_download_edge_cases(self, temp_dir):
        ...
        dataset_oasis2 = {"name": "OASIS2", "base_command": "multiple_aria2c_downloads"}
    ...
    def test_requests_downloader_filename_extraction(self, sample_dataset, temp_dir):
        ...
        with patch("requests.get", return_value=mock_response), patch(
            ...
            downloader = RequestsDownloader(
                sample_dataset, str(temp_dir), url_no_filename
            )
...
class TestFolderStructure:
    ...
    def test_datalad_downloader_downloads_bids_metadata(self, temp_dir):
        ...
        with patch(
            "neurodatahub.downloader.run_command", side_effect=mock_run_command
        ), patch("neurodatahub.downloader.display_info"), patch(
            "neurodatahub.downloader.display_success"
        ), patch(
            "neurodatahub.downloader.display_warning"
        ), patch(
            "os.getcwd", return_value=original_cwd
        ), patch(
            "os.chdir"
