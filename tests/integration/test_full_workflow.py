...
class TestFullWorkflow:
    ...
    def test_list_and_info_workflow(self, integration_setup):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "builtins.open",
            side_effect=lambda f, *args: open(
                integration_setup["datasets_file"], *args
            ),
        ), patch("neurodatahub.cli.console"), patch("neurodatahub.datasets.console"):
    ...
    def test_check_dependencies_workflow(self, integration_setup):
        ...
        with patch("neurodatahub.cli.display_info") as mock_info, patch(
            "neurodatahub.utils.get_dependency_status"
        ) as mock_status, patch("neurodatahub.utils.console"):
    ...
    def test_no_auth_download_workflow(self, integration_setup):
        ...
        ), patch("neurodatahub.downloader.validate_path", return_value=True), patch(
            "neurodatahub.downloader.check_available_space", return_value=True
        ), patch(
            "neurodatahub.downloader.check_dependency", return_value=True
        ), patch(
            "neurodatahub.downloader.run_command", return_value=(0, "success", "")
        ), patch(
            "neurodatahub.cli.get_confirmation", return_value=True
        ), patch(
            "neurodatahub.cli.console"
        ), patch(
            "neurodatahub.datasets.console"
        ), patch(
            "neurodatahub.utils.console"
        ), patch(
            "neurodatahub.downloader.display_info"
        ), patch(
            "neurodatahub.downloader.display_success"
        ), patch(
    ...
    def test_auth_required_download_workflow(self, integration_setup):
        ...
        ), patch("neurodatahub.auth.check_dependency", return_value=True), patch(
            "neurodatahub.auth.run_command", return_value=(0, "configured", "")
        ), patch(
            "neurodatahub.downloader.validate_path", return_value=True
        ), patch(
            "neurodatahub.downloader.check_available_space", return_value=True
        ), patch(
            "neurodatahub.downloader.check_dependency", return_value=True
    ...
    def test_ida_download_workflow(self, integration_setup):
        ...
        ), patch("neurodatahub.ida_flow.check_dependency", return_value=True), patch(
            "neurodatahub.ida_flow.validate_path", return_value=True
        ), patch(
            "neurodatahub.ida_flow.get_confirmation", return_value=True
        ), patch(
            "neurodatahub.ida_flow.get_user_input",
            return_value="https://ida.loni.usc.edu/download/test",
        ), patch(
            "neurodatahub.downloader.run_command", return_value=(0, "success", "")
        ), patch(
            "neurodatahub.cli.get_confirmation", return_value=True
    ...
    def test_dry_run_workflow(self, integration_setup):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
        ...
        ), patch("neurodatahub.cli.console"), patch(
            "neurodatahub.datasets.console"
        ), patch(
            "neurodatahub.downloader.display_info"
        ):
            ...
            result = runner.invoke(
                main, ["pull", "TEST_INDI", str(download_dir), "--dry-run"]
            )
            assert result.exit_code == 0
    ...
    def test_error_recovery_workflow(self, integration_setup):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            ...
            "neurodatahub.downloader.check_dependency", return_value=False
        ), patch(
            "neurodatahub.cli.get_confirmation", return_value=True
        ), patch(
            "neurodatahub.cli.console"
            ...
            "neurodatahub.downloader.display_error"
        ):
    ...
    def test_interrupted_download_workflow(self, integration_setup):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            ...
            "neurodatahub.downloader.run_command", side_effect=KeyboardInterrupt
        ), patch(
            "neurodatahub.cli.get_confirmation", return_value=True
        ), patch(
            "neurodatahub.cli.console"
...
class TestRealDatasetIntegration:
    ...
    def test_real_dataset_validation(self):
        ...
        for dataset_id, dataset in config["datasets"].items():
            ...
            assert (
                dataset["category"] in config["categories"]
            ), f"Dataset {dataset_id} has invalid category {dataset['category']}"

            # Check valid download method
            assert (
                dataset["download_method"] in config["download_methods"]
    ...
    def test_cli_with_real_config(self):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "builtins.open", side_effect=lambda f, *args: open(datasets_file, *args)
        ), patch("neurodatahub.cli.console"), patch("neurodatahub.datasets.console"):
