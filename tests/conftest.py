...
def mock_user_input():
    ...
    with patch("neurodatahub.utils.get_user_input") as mock_input, patch(
        "neurodatahub.utils.get_confirmation"
    ) as mock_confirm:
...
def mock_console():
    ...
    with patch("neurodatahub.utils.console") as mock, patch(
        "neurodatahub.cli.console"
    ) as mock_cli, patch("neurodatahub.datasets.console") as mock_datasets:
        yield mock
...
class MockDatasetManager:
    ...
    def search_datasets(self, query):
        ...
        for dataset_id, dataset in self.datasets.items():
            if (
                query_lower in dataset.get("name", "").lower()
                or query_lower in dataset.get("description", "").lower()
                or query_lower in dataset_id.lower()
            ):
    ...
    def get_datasets_requiring_auth(self):
        ...
        return {k: v for k, v in self.datasets.items() if v.get("auth_required", False)}

    def get_datasets_no_auth(self):
        """Get all datasets not requiring authentication."""
        return {
            k: v for k, v in self.datasets.items() if not v.get("auth_required", False)
        }
    ...
    def get_dataset_stats(self):
        ...
        by_auth = {"required": auth_required, "not_required": total - auth_required}
    ...
    def display_datasets_table(
        self, datasets=None, show_auth=True, show_size=True, detailed=False
    ):
...
def reset_global_state():
    """Reset global state manager before each test to ensure isolation."""
    # Reset global state manager singleton
    import neurodatahub.state as state_module
