...
class Config:
    ...
    DEFAULT_CONFIG = {
        ...
        "aria2": {"max_connections": 16, "split": 16, "max_tries": 5, "retry_wait": 0},
        "ui": {
            "color_output": True,
            "progress_bars": True,
        ...
        "telemetry": {
            "enabled": None,  # None = not asked yet, True/False = user choice
            "endpoint": (
                "https://script.google.com/macros/s/"
                "AKfycbxvtanEmHq2UMUExNo5ELIy1qm0JSt9nRsaqNnFE9PwHHlRp-FDp7JDGlM8FijDkiHE/exec"
            ),
            "timeout": 3,  # seconds
            "rate_limit_per_minute": 10,
        },
        "feedback": {
            "schedule": [1, 3, 10, 30, 50],  # Run counts when to prompt
            "interval_after_50": 50,  # Prompt every N runs after 50th
        },
    }
        ...
        if download_path:
            ...
            try:
                ...
                if not os.access(str(path_obj), os.W_OK):
                    errors.append(
                        f"No write permission for download path: {download_path}"
                    )
            except Exception as e:
