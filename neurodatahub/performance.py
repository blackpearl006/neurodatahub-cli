...
class ParallelDownloadManager:
    ...
    def submit_download(
        self, download_func: Callable, *args, **kwargs
        ...
        with self._lock:
            self.active_downloads[future] = {
                ...
                "function": (
                    download_func.__name__
                    if hasattr(download_func, "__name__")
                    else str(download_func)
                ),
            }
    ...
    def wait_for_downloads(
        self, futures: List[concurrent.futures.Future], timeout: Optional[float] = None
    ) -> Dict[concurrent.futures.Future, Any]:
                ...
                try:
                    ...
                    with self._lock:
                        if future in self.active_downloads:
                            ...
                            logger.info(
                                f"Download completed: {download_info['function']} in {duration:.1f}s"
                            )
...
class ProgressiveDownloader:
    ...
    def __init__(self):
        ...
        self.strategies = [self._try_aria2c, self._try_aws_cli, self._try_requests]
    ...
    def _try_aria2c(self, url: str, target_path: Path, **kwargs) -> bool:
        ...
        returncode, stdout, stderr = run_command(
            cmd, timeout=kwargs.get("timeout", 3600)
        )
...
class ResourceMonitor:
    ...
    def __init__(self):
        ...
        self.stats = {"peak_memory": 0, "peak_cpu": 0, "disk_io": 0, "network_io": 0}
    ...
    def stop_monitoring(self) -> Dict:
        ...
        logger.debug(
            f"Resource monitoring stopped. Peak memory: {self.stats['peak_memory']:.1f} MB"
        )
...
class AsyncDownloadManager:
    ...
    async def download_multiple(
        self, urls_and_paths: List[Tuple[str, Path]]
    ) -> List[bool]:
