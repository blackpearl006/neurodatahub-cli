...
from .exceptions import DataIntegrityError, ValidationError
...
class DatasetValidator:
    ...
    VALID_CATEGORIES = {"indi", "openneuro", "independent", "rbc", "ida"}
    ...
    def _validate_dataset_id(self, dataset_id: str):
        ...
        if len(dataset_id) > 50:
            self.warnings.append(
                f"Dataset ID '{dataset_id}' is very long ({len(dataset_id)} chars)"
            )
    ...
    def _validate_description(self, description: Optional[str]):
        ...
        if len(description) > 1000:
            self.warnings.append(
                f"Dataset description is very long ({len(description)} chars)"
            )
    ...
    def _validate_size(self, size: Optional[str]):
        ...
        if not self.SIZE_PATTERN.match(size):
            self.errors.append(
                f"Invalid size format '{size}'. Expected format: '~100GB' or '2.5TB'"
            )
            return
    ...
    def _validate_download_method(self, download_method: Optional[str]):
        ...
        if download_method not in self.VALID_DOWNLOAD_METHODS:
            self.errors.append(
                f"Invalid download_method '{download_method}'. Must be one of: {self.VALID_DOWNLOAD_METHODS}"
            )
    ...
    def _validate_base_command(self, base_command: Optional[str]):
        ...
        for pattern in suspicious_patterns:
            if re.search(pattern, base_command, re.IGNORECASE):
                self.warnings.append(
                    f"base_command contains potentially unsafe pattern: {pattern}"
                )
                break
    ...
    def _validate_publication(self, publication: Optional[str]):
        ...
        if publication.startswith("http"):
            ...
            if not re.match(r"^10\.\d+/.+", publication):
                self.warnings.append(
                    f"Publication DOI format may be invalid: {publication}"
                )
        else:
    ...
    def _validate_auth_consistency(self, dataset_data: Dict):
        ...
        if auth_required and download_method in no_auth_methods:
            self.warnings.append(
                f"auth_required=true but download_method='{download_method}' typically doesn't require auth"
            )

        if not auth_required and download_method in auth_methods:
            self.warnings.append(
                f"auth_required=false but download_method='{download_method}' typically requires auth"
            )
    ...
    def _validate_command_method_consistency(self, dataset_data: Dict):
        ...
        if download_method in method_patterns:
            ...
            if not re.search(expected_pattern, base_command, re.IGNORECASE):
                self.warnings.append(
                    f"base_command doesn't match download_method '{download_method}'"
                )
...
class FileIntegrityChecker:
    ...
    def check_file_format(
        self, file_path: Path, expected_formats: Optional[Set[str]] = None
    ) -> Dict[str, str]:
    ...
    def validate_dataset_structure(
        self, dataset_path: Path, expected_structure: Optional[Dict] = None
    ) -> Dict:
        ...
        for root, dirs, files in os.walk(dataset_path):
            ...
            for file_name in files:
                ...
                try:
                    ...
                    extension = file_path.suffix.lower()
                    if extension:
                        results["file_types"][extension] = (
                            results["file_types"].get(extension, 0) + 1
                        )
                    else:
                        results["file_types"]["no_extension"] = (
                            results["file_types"].get("no_extension", 0) + 1
                        )

                    # Check for common issues
                    if stat.st_size == 0:
                        results["issues"].append(
                            f"Empty file: {file_path.relative_to(dataset_path)}"
                        )

                    if not os.access(file_path, os.R_OK):
                        results["issues"].append(
                            f"Unreadable file: {file_path.relative_to(dataset_path)}"
                        )
        ...
        if expected_structure:
            self._validate_against_expected_structure(
                dataset_path, expected_structure, results
            )

        return results

    def _validate_against_expected_structure(
        self, dataset_path: Path, expected: Dict, results: Dict
    ):
...
def validate_datasets_config(config_file: Path) -> Tuple[bool, List[str]]:
    ...
    for dataset_id, dataset_data in datasets.items():
        if not validator.validate_dataset(dataset_id, dataset_data):
            issues.extend(
                [f"Dataset {dataset_id}: {error}" for error in validator.errors]
            )
            issues.extend(
                [f"Dataset {dataset_id}: {warning}" for warning in validator.warnings]
            )
    ...
    for category_id, category_data in categories.items():
        ...
        if missing_cat_fields:
            issues.append(
                f"Category {category_id} missing fields: {missing_cat_fields}"
            )
    ...
    for method_id, method_data in methods.items():
        ...
        if missing_method_fields:
            issues.append(
                f"Download method {method_id} missing fields: {missing_method_fields}"
            )
