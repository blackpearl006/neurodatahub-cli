...
class ProgressTracker:
    ...
    def start_download(
        ...
        if dataset_id in self._progress_data:
            ...
            if existing.is_completed:
                ...
                logger.info(
                    f"Resuming download {dataset_id} from {existing.progress_percentage:.1f}%"
                )
    ...
    def update_progress(
        self, dataset_id: str, downloaded_size: int, total_size: Optional[int] = None
    ):
    ...
    def show_progress_summary(self):
        ...
        for progress in downloads[:20]:  # Show last 20
            ...
            if progress.start_time:
                start_text = datetime.fromtimestamp(progress.start_time).strftime(
                    "%Y-%m-%d %H:%M"
                )
            else:
                start_text = "-"
...
class RichProgressManager:
    ...
    def add_download(self, dataset_id: str, total_size: Optional[int] = None) -> TaskID:
        ...
        task_id = self.progress.add_task(
            f"Downloading {dataset_id}", total=total_size, start=False
        )
    ...
    def fail_download(self, dataset_id: str, error: str):
        ...
        if dataset_id in self._tasks:
            ...
            self.progress.update(
                task_id, description=f"[✗] {dataset_id} failed: {error}"
            )
