...
class DatasetManager:
    ...
    def display_datasets_table(
        self, datasets: Optional[Dict[str, Dict]] = None, detailed: bool = False
    ):
        ...
        for dataset_id, dataset in sorted(datasets.items()):
            ...
            has_metadata = (
                has_metadata_urls
                or has_metadata_files
                or has_metadata_note
                or has_metadata_command
            )
            ...
            if detailed:
                ...
                desc_short = (
                    (description[:50] + "...") if len(description) > 53 else description
                )
        ...
        total = len(datasets)
        auth_required = len(
            [d for d in datasets.values() if d.get("auth_required", False)]
        )
        ...
        with_metadata = len(
            [
                ...
                if d.get("metadata_urls")
                or d.get("metadata_files")
                or d.get("metadata_note")
                or d.get("metadata_command")
            ]
        )
    ...
    def display_categories_table(self):
        ...
        for cat_id, category in self.categories.items():
            ...
            count = len(
                [d for d in self.datasets.values() if d.get("category") == cat_id]
            )
